...
export default function facets(request, queryConfig) {
    ...
    Object.entries(queryConfig.facets).forEach(([facetName, facet]) => {
        ...
        processedFacets.push(`{ field: "${facetName}", type: ${facetTypes[facet.type]}, disjunctive:  ${Boolean(facet.disjunctive)} ${facet.size ? `, max: ${facet.size},` : ''} ${selections.length > 0 ? `, selections: [${selections.join(',')}]` : ''} }`);
    });
